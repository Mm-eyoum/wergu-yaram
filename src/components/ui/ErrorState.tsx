...
export function ErrorState({ title, message, onRetry }: ErrorStateProps) {
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-3 rounded-3xl border border-dashed border-danger/30 bg-white px-6 py-12 text-center">
      ...
      <h3 className="text-base font-bold text-text-primary">{title ?? t("states.errorTitle")}</h3>
      <p className="max-w-sm text-sm text-text-secondary">{message ?? t("states.errorMessage")}</p>
      {onRetry && (
        <button
          type="button"
          onClick={onRetry}
